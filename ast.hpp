...
/**
 * Implementation of an adaptive suffix tree.
 * It is an extension of an addaptive radix tree.
 */
#pragma once
...
public:
    ...
    KeySpan(const u8* const data, usize size) noexcept : m_data{data}, m_size{size + 1} {}
...
public:
    ...
    //
    [[nodiscard]] usize common_header_prefix(const KeySpan& key, usize depth) const noexcept
    {
    ...
    //
    [[nodiscard]] usize common_prefix(const AST& ctx, const KeySpan& key,
...
public:
    ...
    Node4(const KeySpan& key, KeyRef value, usize depth, const KeySpan& old_key, KeyRef old_value)
        : Node{Node4_t}
    {
    ...
    //
    Node4(const KeySpan& key, KeyRef value, usize depth, KeySpan old_key, const Leaf& old_leaf)
        : Node{Node4_t}
    {
        for (m_prefix_len = 0; key[depth] == old_key[depth]; ++m_prefix_len, ++depth)
            if (m_prefix_len < max_prefix_len)
    ...
    Node4(const AST& ctx, const KeySpan& key, KeyRef value, usize depth, Node& node, usize cp_len)
        : Node{Node4_t}
    {
...
public:
    ...
    static constexpr usize init_size = 128;
    ...
    {
        usize size = sizeof(Data) + m_capacity * sizeof(KeyValue*);
        for (u32 i = 0; i < m_capacity; ++i)
            if (m_kv[i] != nullptr)
...
public:
    ...
    [[nodiscard]] KeyValue* const search(const u8* const data, usize size) const noexcept
    {
        const KeySpan key{data, size};
        return search(key);
    }
    ...
    [[nodiscard]] const std::vector<KeyValue*>
    search_suffix(const u8* const suffix, usize prefix_size,
                  usize limit = std::numeric_limits<usize>::max()) const noexcept
    {
    ...
    //
    template<class Pred>
    void search_prefix_while(const std::string& prefix, Pred pred) const noexcept
    {
        const KeySpan key{(const u8* const)prefix.data(), prefix.size()};
        search_prefix_while((const u8* const)prefix.data(), prefix.size(), pred);
    }
    ...
    //
    template<class Pred>
    void search_prefix_while(const u8* const prefix, usize prefix_size, Pred pred) const noexcept
    {
        const KeySpan key{prefix, prefix_size};
        search_prefix_while(key, pred);
    }
    ...
    usize size_in_bytes() const noexcept
    {
        usize size = sizeof(entry_ptr); // Root entry ptr.
...
private:
    ...
    //
    void insert_at_entry(entry_ptr& entry, const KeySpan& key, KeyRef value, usize depth) noexcept
    {
     ...
     */
    void insert_at_leaf(entry_ptr& entry, const KeySpan& key, KeyRef value, usize depth) noexcept
    {
     ...
     * full node prefix.
     */
    void insert_at_node(entry_ptr& entry, const KeySpan& key, KeyRef value, usize depth,
                        usize cp_len) noexcept
    {
        Node* node = entry.node_ptr();
        assert(cp_len != node->m_prefix_len);
    ...
    //
    void insert(entry_ptr& entry, const KeySpan& key, KeyRef value, usize depth) noexcept
    {
        assert(entry);
        ...
        Node* node = entry.node_ptr();
        usize cp_len = node->common_prefix(*this, key, depth);
    ...
    void erase_at_leaf(entry_ptr& entry, Leaf* leaf, const KeySpan& key, usize depth,
                       u32 data_idx) noexcept
    {
    ...
    KeyValue* const search(const entry_ptr& entry, const KeySpan& key, usize depth) const noexcept
    {
        if (entry.ref()) {
    ...
    const std::vector<KeyValue*> search_prefix(const KeySpan& key, usize limit) const noexcept
    {
        if (!m_root)
            return {};
    ...
    void search_prefix(const entry_ptr& entry, const KeySpan& prefix, usize depth,
                       std::unordered_set<KeyValue*>& result, usize limit) const noexcept
    {
        if (entry.ref()) {
            KeyRef ref = entry.key_ref();
        ...
        Node* node = entry.node_ptr();
        usize cp_len = node->common_prefix(*this, prefix, depth);
    ...
    const std::vector<KeyValue*> search_suffix(const KeySpan& key, usize limit) const noexcept
    {
        if (!m_root)
            return {};
    ...
    void search_suffix(const entry_ptr& entry, const KeySpan& suffix, usize depth,
                       std::vector<KeyValue*>& result, usize limit) const noexcept
    {
        if (entry.ref()) {
            KeyRef ref = entry.key_ref();
            KeySpan key = m_data[ref];
        ...
        Node* node = entry.node_ptr();
        usize cp_len = node->common_header_prefix(suffix, depth);
    ...
    template<class Pred>
    void search_prefix_if(const entry_ptr& entry, const KeySpan& prefix, usize depth,
                          std::unordered_set<KeyValue*>& result, Pred pred,
                          usize limit) const noexcept
    {
        if (entry.ref()) {
            KeyRef ref = entry.key_ref();
    ...
    template<class Pred>
    void search_prefix_while(const KeySpan& prefix, Pred pred) const noexcept
    {
        search_prefix_while(m_root, prefix, 0, pred);
    }

    template<class Pred>
    void search_prefix_while(const entry_ptr& entry, const KeySpan& prefix, usize depth,
                             Pred pred) const noexcept
    {
        ...
        if (depth + cp_len == prefix.size() - 1) {
            for_each(entry, [&](const entry_ptr& ent) {
                if (ent.ref()) {
                    auto& v = m_data[ent.key_ref().idx()]->value();
                    if (!pred(v))
                        return false;
                }
